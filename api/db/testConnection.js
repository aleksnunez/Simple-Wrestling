...
const testConnect = () => {
  ...
  const CREATE_TABLE_LOGIN =
    "CREATE TABLE coach(id SERIAL PRIMARY KEY, email VARCHAR(40) NOT NULL, password VARCHAR(40) NOT NULL";
  const test =
  ...
  let createLoginTableTest = () => {
    pool.query(
      "CREATE TABLE coach(id SERIAL PRIMARY KEY, firstName VARCHAR(40) NOT NULL, lastName VARCHAR(40) NOT NULL)",
      (err, res) => {
        console.log(res);
        pool.end();
      }
